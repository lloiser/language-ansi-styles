...
export function convert (line, ruleStack, firstLine, registry) {
  ...
  while (text.length > 0) {
    ...
    codes.split(';').forEach((v) => {
      processCode(state, parseInt(v || '0', 10))
    })
...
const codes = {
  ...
  2: (state, code) => {
    // TODO: faint, decreased intensity
  },
  3: (state, code) => {
  ...
  22: (state, code) => {
    // TODO: normal color or intensity
  },
  23: (state, code) => {
  ...
  // TODO: color
  30: (state, code) => {
    openClosable(state, code, 'color.fg.' + Colors[code - 30])
  },
  31: 30,
  32: 30,
  33: 30,
  ...
  40: (state, code) => {
    openClosable(state, code, 'color.bg.' + Colors[code - 40])
  },
  41: 40,
  42: 40,
  43: 40,
  ...
  90: (state, code) => {
    openClosable(state, code, 'color.fg.bright.' + Colors[code - 90])
  },
  91: 90,
  92: 90,
  93: 90,
  ...
  // 98, 99: nothing
  100: (state, code) => {
    openClosable(state, code, 'color.bg.bright.' + Colors[code - 100])
  },
  101: 100,
  102: 100,
  103: 100,
...
function processCode (state, code) {
  const fn = codes[code]
  const t = typeof fn
  if (t === 'function') {
    fn(state, code)
...
function openClosable (state, code, scope) {
  ...
  if (scope.startsWith('color.fg.')) {
    if (state.colorFG) {
      close(state, state.colorFG)
    }
    state.colorFG = scope
  } else if (scope.startsWith('color.bg.')) {
    if (state.colorBG) {
      close(state, state.colorBG)
    }
    state.colorBG = scope
  }
