...
export function convert (line, ruleStack, firstLine, registry, hideEscapeSequences) {
  const state = {
    registry,
    tags: [],
    openTags: {},
  ...
  matches.forEach((match) => {
    ...
    // add tags for the codes
    const scope = 'codes' + (hideEscapeSequences ? '.hidden' : '')
    open(state, scope)
    state.tags.push(len)
    close(state, scope)
...
function safeClose (state, scope) {
  const ignoreTagsForClose = [
    ...
    state.registry.startIdForScope('codes'),
    state.registry.startIdForScope('codes.hidden')
  ]
